...
| #   | Item                                                                                                                   | Where                                            | Action                                                                                                                                        |
| --- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| 🟥  | **Canonical URL** is a placeholder: `https://kjolstad-lab.stanford.edu`                                                | `src/config/site.ts` → `SITE.url`                | Replace with the real deploy domain. Feeds `canonical`, `sitemap`, `robots.txt`, OG URLs.                                                     |
| 🟥  | **Lab name** "Kjolstad Lab" is temporary (permanent name not yet chosen)                                               | `src/config/site.ts` → `SITE.name` / `shortName` | Update in one place when the name is decided. Also regenerate the favicon/OG (`pnpm run generate:assets`) if the wordmark text should change. |
